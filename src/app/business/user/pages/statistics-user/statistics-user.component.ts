...
import { QuizzesUserComponent } from '../quizzes-user/quizzes-user.component';
...
export class StatisticsUserComponent implements OnInit {
  chart: any;
  userId = 1;
  questionDetails: QuizzesQuestion[] = [];
  quizzes: Quiz[] = [];
  quizId: number;
  ...
  onUpdateList() {
    ...
    this.qqService.getPercentageCorrectAnswers(this.quizId).subscribe({
      next: (data) => {
        ...
        this.updateChart(); // Call updateChart to refresh the chart with new avgCorrectAnswers
      }
    });

    this.qqService.listQuizzesQuestionByQuizId(this.quizId).subscribe({
      next: (data: QuizzesQuestion[]) => {
